...
const normalizeRaiddEntry = (entry) => {
    ...
    if (typeof entry === "object" && !Array.isArray(entry)) {
        if (entry.data) {
            return { kind: "line", text: String(entry.data).trim() };
        }
...
const normalizeVendor = (vendor) => {
    return {
        id: String(vendor?.id || ""),
        name: vendor?.clientName || vendor?.name || fallback,
        ...
        ai: {
            ...
            lessonsLearned: Array.isArray(vendor?.lessonsLearned) ? vendor.lessonsLearned : [],
            actionPoints: Array.isArray(vendor?.actionPoints) ? vendor.actionPoints : [],
            discussionPoints: Array.isArray(vendor?.discussionPoints) ? vendor.discussionPoints : [],
            raiddFlags: {
                risks: normalizeRaiddSection(vendor?.raiddData?.risks),
                issues: normalizeRaiddSection(vendor?.raiddData?.issues),
                assumptions: normalizeRaiddSection(vendor?.raiddData?.assumptions),
                decisions: normalizeRaiddSection(vendor?.raiddData?.decisions),
                dependencies: normalizeRaiddSection(vendor?.raiddData?.dependencies),
            },
        },
    };
};
...
export default function ViewVendor() {
    ...
    const {
        ...
        queryFn: async () => {
            try {
                return await apiGet(`/api/project-manager/client-management/${id}`);
            } catch {
